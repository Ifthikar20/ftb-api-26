"""
Framework-free LLM logic for the intelligence service.

Single source of truth for brand/sentiment/issue extraction and SERP
relevance judging. Imported two ways:

- by services.intelligence.main (the FastAPI app) in production, and
- by apps/citations/services/source_sentiment.py directly when
  INTELLIGENCE_SERVICE_URL is unset (dev, tests, CI).

No Django, no FastAPI imports here. Credentials and model names are
plain arguments. Every function returns an envelope:

    {"result": <caller-facing payload>,
     "usage": {"input_tokens": int, "output_tokens": int} | None,
     "duration_ms": int, "model": str, "prompt_version": str}

``usage`` is None exactly on the paths where no billable API call
completed (missing key, empty content, SDK missing, API exception) so
the Django facade can decide whether to record cost.
"""
...
def _envelope(result, *, usage=None, duration_ms=0, model=""):
    return {
        "result": result,
        "usage": usage,
...
def analyze_content(
    ...
    *,
    ...
    """Extract brand sentiment/issues from one result's content.

    ``result`` is {"brands": [...], "target_brand_present": bool,
    "relevant_to_query": bool, "summary": str} or
    {"brands": [], "error": "..."} when unavailable/failed.
    """
    if not api_key:
        return _envelope({"brands": [], "error": "no_api_key"}, model=model_name)
    if not (text or "").strip():
        return _envelope({"brands": [], "error": "empty_content"}, model=model_name)

    try:
        import anthropic
    except ImportError:
        return _envelope({"brands": [], "error": "anthropic_not_installed"}, model=model_name)
    ...
    try:
        ...
        return _envelope({"brands": [], "error": str(exc)[:200]}, model=model_name)
    ...
    try:
        ...
        return _envelope(
            {"brands": [], "error": "non_json_response"},
            usage=usage, duration_ms=duration_ms, model=model_name,
        )
    ...
    return _envelope(result, usage=usage, duration_ms=duration_ms, model=model_name)
...
def assess_serp_relevance(
    ...
    *,
    ...
    ``result`` maps rank (int) to {"relevant": bool, "reason": str}.
    ...
    if not api_key or not items:
        return _envelope(all_relevant, model=model_name)
    ...
    try:
        ...
        return _envelope(all_relevant, usage=usage, duration_ms=duration_ms, model=model_name)
    ...
    return _envelope(verdicts, usage=usage, duration_ms=duration_ms, model=model_name)
